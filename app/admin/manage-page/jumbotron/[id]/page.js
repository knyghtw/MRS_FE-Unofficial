...
import showJumbotron from "@/api/jumbotrons/show";
import updateJumbotron from "@/api/jumbotrons/update";

export default function ManageJumbotronLayout({ params }) {
  ...
  const handleForm = async () => {
    if (!hasUnsavedChanges) {
      ...
      if (jumbotron_img !== null) {
        formData.append("JumbotronImage", jumbotron_img);
      }

      await updateJumbotron(id, formData);
  ...
  const fetchData = async () => {
    setIsClient(false);
    const response = await showJumbotron(id);

    setJumbotronImgPreview(
      `${baseUrl}${response.data.JumbotronImage}`
    );
    setJumbotronTitle(response.data.JumbotronTittle);
    setJumbotronDesc(response.data.JumbotronDescription);
    setIsClient(true);
  };
